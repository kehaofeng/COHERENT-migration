...
if not coherent_path:
    raise RuntimeError("COHERENT_PATH must point to the COHERENT repository root")
...
class ActionPublishandResultSubscribe(Node):
    def __init__(self, task_name):
        super().__init__("coherent_action_publisher")
        self.task_name = task_name
        ...
        self._sub = self.create_subscription(Result, "resultTopic", self.callback, qos)
        self._pub = self.create_publisher(Action, "actionTopic", qos)
